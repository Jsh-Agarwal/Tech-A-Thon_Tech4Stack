...
def find_path_bfs(mapT, start_point, end_point, box_radius=30):
    ...
    start_idx = np.argmin(np.sqrt((start_y-y0)**2 + (start_x-y0)**2))
    ...
    while True:
        ...
        for new_x, new_y in zip(xs+x-1, ys+y-1):
            if 0 <= new_x < 1000 and 0 <= new_y < 1000:  # Boundary check
                pts_x.append(new_x)
                pts_y.append(new_y)
                pts_c.append(ct+1)
...
def find_path_dijkstra(mapT, start_point, end_point, box_radius=30):
    ...
    while pq:
        ...
        for dx, dy in zip(xmesh, ymesh):
            ...
            if 0 <= new_x < 1000 and 0 <= new_y < 1000 and _mapt[new_y, new_x] == 0:  # Boundary check
                new_cost = ct + 1
